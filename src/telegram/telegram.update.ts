...
export class TelegramUpdate {
  ...
  @Command('deals_on')
  async onDealsOn(@Ctx() ctx: Context) {
    const from = ctx.from;
    if (!from) return;
    ...
    await this.deals.setDealSubscription(user.id, true);
    await ctx.reply(
      [
        '✅ Bạn đã bật nhận deal hot từ ChotDeal.',
        '',
        'Khi có deal Shopee/Taobao tốt, bot sẽ gửi thông báo cho bạn.',
        'Bạn có thể gõ /deals_off bất cứ lúc nào để tắt.',
      ].join('\n'),
    );
  }

  @Command('deals_off')
  async onDealsOff(@Ctx() ctx: Context) {
    const from = ctx.from;
    if (!from) return;
    ...
    await this.deals.setDealSubscription(user.id, false);
    await ctx.reply(
      [
        '🚫 Bạn đã tắt nhận deal hot.',
        '',
        'Bot vẫn hoạt động bình thường khi bạn dán link để lấy cashback.',
        'Muốn bật lại, gõ /deals_on.',
      ].join('\n'),
    );
  }

  @Command('deal_settings')
  async onDealSettings(@Ctx() ctx: Context) {
    const from = ctx.from;
    if (!from) return;
    ...
    const status = await this.deals.getDealSubscriptionStatus(user.id);
    await ctx.reply(
      [
        '⚙️ Cài đặt nhận deal',
        '',
        `Trạng thái: ${status.isEnabled ? '✅ Đang bật' : '🚫 Đang tắt'}`,
        '',
        '/deals_on - bật nhận deal hot',
